...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 新增：點擊「現在播放歌曲」標題時，進入歌曲詳情頁面
  async function handleNowPlayingTitleClick(e) {
    e.stopPropagation();
    e.preventDefault();
    
    console.log("點擊了現在播放標題");
    console.log("nowPlayingSongList:", nowPlayingSongList);
    console.log("nowPlayingSongIndex:", nowPlayingSongIndex);
    console.log("audioPlayer.src:", audioPlayer ? audioPlayer.src : "audioPlayer 不存在");
    
    // 檢查是否有正在播放的歌曲
    if (nowPlayingSongList.length > 0 && nowPlayingSongIndex >= 0 && nowPlayingSongIndex < nowPlayingSongList.length) {
      const currentSong = nowPlayingSongList[nowPlayingSongIndex];
      console.log("當前歌曲:", currentSong);
      
      if (currentSong && currentSong.cid) {
        // 獲取歌曲的封面資訊
        let coverUrl = currentSong.coverUrl || currentSong.albumCover || "";
        let coverDeUrl = currentSong.coverDeUrl || currentSong.albumCover || "";
        
        // 如果沒有封面資訊，嘗試從專輯資訊中獲取
        if (!coverUrl || !coverDeUrl) {
          try {
            console.log("嘗試獲取專輯封面資訊...");
            // 先獲取歌曲詳情以取得專輯ID
            const songResponse = await fetch(`https://monstersiren-web-api.vercel.app/api/song/${currentSong.cid}`);
            if (songResponse.ok) {
              const { data: songDetails } = await songResponse.json();
              const albumCid = songDetails.albumCid;
              if (albumCid) {
                // 獲取專輯詳情以取得封面
                const albumResponse = await fetch(`https://monstersiren-web-api.vercel.app/api/album/${albumCid}/detail`);
                if (albumResponse.ok) {
                  const { data: albumDetails } = await albumResponse.json();
                  coverUrl = coverUrl || albumDetails.coverUrl || "";
                  coverDeUrl = coverDeUrl || albumDetails.coverDeUrl || "";
                }
              }
            }
          } catch (error) {
            console.warn("獲取專輯封面資訊時出錯:", error);
          }
        }
        
        console.log("準備顯示歌曲詳情頁面，歌曲ID:", currentSong.cid);
        // 顯示歌曲詳情頁面
        fetchSongDetails(currentSong.cid, coverUrl, coverDeUrl);
      } else {
        console.warn("當前歌曲沒有有效的 cid");
      }
    } else {
      console.warn("沒有正在播放的歌曲或索引無效");
    }
  }
  
  if (nowPlayingTitle) {
    nowPlayingTitle.style.cursor = "pointer";
    nowPlayingTitle.onclick = handleNowPlayingTitleClick;
    
    // 也為容器添加點擊事件，以防標題元素被動畫覆蓋
    const marqueeContainer = document.querySelector(".now-playing-marquee-container");
    if (marqueeContainer) {
      marqueeContainer.style.cursor = "pointer";
      marqueeContainer.onclick = handleNowPlayingTitleClick;
    }
  }
  ...
  albums.forEach((album) => {
    ...
    setTimeout(() => {
      if (marqueeContent.scrollWidth > container.clientWidth) {
        ...
        marqueeContent.style.animation = "marquee 5s linear infinite";
      } else {
  ...
  setTimeout(() => {
    ...
    titles.forEach((titleElement) => {
      ...
      if (titleElement.scrollWidth > titleElement.clientWidth) {
        ...
        const duration = originalText.length / 15; // 根據文字長度調整滾動速度（數值越大速度越快）
...
function syncLyrics(currentTime, forceScroll = false) {
  ...
  if (activeLineIndex !== -1 && lines[activeLineIndex]) {
    ...
    requestAnimationFrame(() => {
      if (lyricsContainer && !isUserScrolled) {
...
function updateNowPlayingTitle(name) {
    ...
    setTimeout(() => {
        ...
        if (hasOverflow) {
            ...
            // 動態調整動畫時間，讓長文字滾得慢一點
            const duration = nowPlayingTitle.scrollWidth / 60; // 調整 60 這個值可以改變滾動速度（數值越大速度越快）
            nowPlayingTitle.style.animation = `marquee ${duration < 5 ? 5 : duration}s linear infinite`;
        }
    }, 100);
}
