    ...
    <section class="recruit-console">
      ...
      <div v-else class="tag-groups">
        <section v-for="group in tagGroups" :key="group.id" class="tag-group">
          <header><i :class="group.icon"></i><span>{{ group.label }}</span></header>
          <div class="tag-grid">
        ...
        <article v-for="result in combinations" :key="result.key" class="combination-row" :class="`guarantee-${result.minRarity}`">
          ...
          <div class="combination-body">
            ...
            <div class="operator-strip">
              <div v-for="operator in result.operators" :key="operator.id" class="operator-token" :title="operator.name">
...
import { fetchRecruitReleaseDates, getProxyImageUrl } from '../services/api.js';
...
const loadData = async () => {
  loading.value = true; error.value = ''; selectedTags.value = [];
  try {
    const base = getRecruitmentGameDataExcelBase(server.value, locale.value);
    const [characterResponse, gachaResponse, releases] = await Promise.all([
      fetch(`${base}/character_table.json`), fetch(`${base}/gacha_table.json`),
      server.value === 'tw' ? fetchRecruitReleaseDates('tw') : Promise.resolve([]),
    ]);
    if (!characterResponse.ok || !gachaResponse.ok) throw new Error('GameData unavailable');
    const [characters, gacha] = await Promise.all([characterResponse.json(), gachaResponse.json()]);
    const roster = parseRecruitRoster(gacha.recruitDetail);
    const twIds = new Set(releases.filter((row) => Date.parse(row.release_at) <= Date.now()).map((row) => row.character_id).filter(Boolean));
    const twNames = new Set(releases.filter((row) => Date.parse(row.release_at) <= Date.now()).flatMap((row) => [row.operator_name, row.cn_name, row.tw_name].map(normalizeName)));
    operators.value = Object.entries(characters).filter(([id, char]) => {
      if (!roster.has(normalizeName(char.name))) return false;
      if (server.value === 'tw' && twIds.size && !twIds.has(id) && !twNames.has(normalizeName(char.name))) return false;
      return char.profession !== 'TOKEN' && char.profession !== 'TRAP';
    }).map(([id, char]) => ({ id, name: char.name, rarity: parseRarity(char.rarity), profession: char.profession, position: char.position, abilities: char.tagList || [] }));
  } catch (cause) { console.error(cause); error.value = copy.value.loadError; operators.value = []; }
...
const abilityTags = computed(() => [...new Set(operators.value.flatMap((item) => item.abilities))].sort().map((value) => ({ key: `ability:${value}`, type: 'ability', value, label: value })));
